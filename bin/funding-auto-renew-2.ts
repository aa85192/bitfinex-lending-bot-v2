...
export async function main (): Promise<void> {
  ...
  const targetRate = frrHighRate
    ? frr // FRR >= 13.95% APR：直接用 FRR 利率
    : _.clamp(Number(ctxBs.targetRate) / 1e8, cfg.rateMin, cfg.rateMax)
  const target = {
    rate: targetRate,
    period: frrHighRate ? 120 : rateToPeriod(cfg.period, targetRate), // FRR >= 13.95% APR：固定 120 天
  }
  if (frrHighRate) loggers.log(`FRR (${rateStringify(frr)}) >= 13.95% APR，使用 FRR 利率並設定 120 天`)
